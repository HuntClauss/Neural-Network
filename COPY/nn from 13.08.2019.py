...
class NeuralNetwork:
    ...
    def train(self, input_arr, target_arr):
        ...
        gradients = _normalize(outputs, True)
        ...
        nodes_values[0] = _normalize(nodes_values[0])
        ...
        for i in range(1, len(nodes_values)):
            ...
            nodes_values[i] = _normalize(nodes_values[i])
        ...
        for i in range(len(nodes_values)-1, -1, -1):
            ...
            gradients = _normalize(nodes_values[i], True)
            gradients = np.multiply(gradients, current_error)
            gradients = np.multiply(gradients, self.learning_rate)
...
nn = NeuralNetwork(28*28, [30, 30, 30], 2)
