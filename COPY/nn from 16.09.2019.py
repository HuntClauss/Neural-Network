...
class NeuralNetwork:
    ...
    def predict(self, input_arr, chance=False, t=False):
        inputs = np.array(input_arr, ndmin=2, dtype="float64")
    ...
    def train(self, input_arr, target_arr):
        inputs = np.array(input_arr, ndmin=2, dtype="float64")
        targets = np.array(target_arr, ndmin=2, dtype="float64")
        # inputs = _sigmoid(inputs)
        ...
        self.weights[-1] = np.add(self.weights[-1], weight_deltas)
...
nn = NeuralNetwork(w*h, [75], 2, learning_rate=0.1)
...
# path = r'face.jpg'
# data = img2array(path)
# print('Face', nn.predict(data, chance=True))

# path = r'cat.jpg'
# data = img2array(path)
# print('Animal', nn.predict(data, chance=True))

path = r'male.jpg'
data = img2array(path)
print('Male Test', nn.predict(data, chance=True))

path = r'Female.jpg'
data = img2array(path)
print('Female Test', nn.predict(data, chance=True))
